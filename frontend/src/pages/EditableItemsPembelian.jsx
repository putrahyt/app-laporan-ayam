...
export default function EditableItemsPembelian({ row, onSaved }) {
  ...
  const simpan = async () => {
    ...
    if (!adaItem && !adaKotak) {
      Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Minimal isi satu item atau satu kotak!', confirmButtonColor: '#dc2626' });
      return;
    }
    if (adaItem && itemsYangDiisi.some(i => !i.jumlah)) {
      Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Jumlah item wajib diisi!', confirmButtonColor: '#dc2626' });
      return;
    }
    if (adaKotak && kotakList.some(k => !k.jumlah)) {
      Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Jumlah kotak wajib diisi!', confirmButtonColor: '#dc2626' });
      return;
    }
    ...
    try {
      ...
      await axios.patch(`${API}/pembelian/${row.id}`, {
      ...
      Swal.fire({ icon: 'error', title: 'Gagal!', text: 'Terjadi kesalahan saat menyimpan.', confirmButtonColor: '#dc2626' });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div style={{ position: 'relative', display: 'inline-block' }}>
      ...
      {open && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.25)' }}>
          <div ref={popupRef} style={{ background: 'white', borderRadius: 14, padding: '18px 20px', boxShadow: '0 8px 40px rgba(220,38,38,0.15)', width: 620, maxWidth: '95vw', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ fontWeight: 700, fontSize: 13, color: '#dc2626', marginBottom: 12 }}>✏️ Edit Item & Harga — {row.nama}</div>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <div key={idx} style={{ display: 'flex', gap: 7, marginBottom: 7, alignItems: 'center' }}>
                    <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#fef2f2', color: '#dc2626', fontSize: 11, fontWeight: 700, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {idx + 1}
                    </div>
                    ...
                    <div style={{ width: 100, fontSize: 12, fontWeight: 600, color: sub > 0 ? '#dc2626' : '#94a3b8', textAlign: 'right' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4, paddingLeft: 30 }}>
                <button onClick={tambahItem}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: '1.5px dashed #dc2626', color: '#dc2626', borderRadius: 7, padding: '4px 12px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                  + Tambah Item
                </button>
                <div style={{ fontSize: 12, color: '#475569', textAlign: 'right' }}>
                  Jml item: <b>{totalItemSaja}</b> &nbsp;|&nbsp; Total harga: <b style={{ color: '#dc2626' }}>Rp {totalHargaItem.toLocaleString('id-ID')}</b>
                </div>
              </div>
            </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6, paddingLeft: 30 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <input type="number" placeholder="0" min="0"
                      ...
                      style={{ width: 120, padding: '5px 7px', border: '2px solid #dc2626', borderRadius: 7, fontSize: 13, textAlign: 'right', fontWeight: 600 }} />
                  </div>
                  <div style={{ fontSize: 12, color: '#475569', textAlign: 'right' }}>
                    Net kotak: <b>{netKotak}</b> &nbsp;|&nbsp; Total harga: <b style={{ color: '#dc2626' }}>Rp {totalHargaKotak.toLocaleString('id-ID')}</b>
                  </div>
                </div>
              </div>
            )}
            ...
            <button onClick={tambahKotak}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: '1.5px dashed #d97706', color: '#d97706', borderRadius: 7, padding: '4px 12px', cursor: 'pointer', fontSize: 12, fontWeight: 600, marginBottom: 12 }}>
              + Tambah Kotak
            </button>

            {/* Ringkasan total */}
            <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 8, padding: '8px 14px', marginBottom: 12 }}>
              <span style={{ fontSize: 13, color: '#475569' }}>
                Total keseluruhan item: <b style={{ color: '#1e293b' }}>{jumlahTotal}</b>
                &nbsp;&nbsp;|&nbsp;&nbsp;
                Total keseluruhan harga: <b style={{ color: '#dc2626' }}>Rp {totalKeseluruhan.toLocaleString('id-ID')}</b>
              </span>
            </div>

            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={simpan} disabled={saving}
                style={{ flex: 1, padding: '9px 0', background: saving ? '#fca5a5' : '#dc2626', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}>
