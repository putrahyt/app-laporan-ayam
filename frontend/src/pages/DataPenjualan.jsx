...
export default function DataPenjualan() {
    ...
    const res = await axios.get(`${API}/penjualan/all?${params}`);
    ...
    const res = await fetch(`${API}/export/semua?${params}`);
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `data-penjualan.xlsx`;
    a.click();
  ...
  return (
    <div>
        ...
        {/* <div style={{ background: 'white', borderRadius: 12, padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', borderLeft: '4px solid #059669' }}>
            <div style={{ fontSize: 13, color: '#64748b', fontWeight: 600, marginBottom: 6 }}>Total Keseluruhan Penjualan Ayam</div>
            <div style={{ fontSize: 24, fontWeight: 700, color: '#047857' }}>Rp {grandTotal.toLocaleString('id-ID')}</div>
            <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>dari semua transaksi</div>
        </div> */}
        </div>

        <div className="card">
        <h2>📊 Data Penjualan — {total} transaksi</h2>
