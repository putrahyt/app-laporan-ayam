...
export default function DetailPenjualan({ id, onKembali }) {
  const [data, setData] = useState(null);

  useEffect(() => {
    axios.get(`${API}/penjualan/${id}`).then(r => setData(r.data));
  }, [id]);
  ...
  return (
    <div className="card" style={{ maxWidth: 780 }}>
      <h2>🔍 Detail Penjualan — {data.nama}</h2>
      ...
      <table style={{ marginBottom: 20, width: '100%' }}>
        <tbody>
          ...
          <tr>
            <td style={{ color: '#475569', padding: '8px 0', verticalAlign: 'top' }}>Nama</td>
          ...
          {items.length > 0 && (
            <tr>
              ...
              <td>
                ...
                {items.map((item, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', gap: 6, marginBottom: 5, alignItems: 'center' }}>
                      <span style={{ width: 20, height: 20, borderRadius: '50%', background: '#eff6ff', color: '#2563eb', fontSize: 10, fontWeight: 700, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>{i + 1}</span>
                      ...
                      <span style={{ width: 45, background: '#eff6ff', color: '#2563eb', padding: '3px 6px', borderRadius: 6, fontSize: 13, fontWeight: 600, textAlign: 'center' }}>{item.satuan}</span>
                      ...
                      <span style={{ width: 120, fontSize: 13, fontWeight: 700, color: sub > 0 ? '#059669' : '#94a3b8', textAlign: 'right' }}>
                ...
                {/* Subtotal item */}
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 4, paddingRight: 0, gap: 8 }}>
                  <span style={{ fontSize: 12, color: '#475569' }}>Jumlah item: <b>{items.reduce((s,i) => s + (Number(i.jumlah)||0), 0)}</b></span>
                  <span style={{ fontSize: 12, color: '#475569', marginLeft: 16 }}>Total harga: <b style={{ color: '#059669' }}>Rp {totalHargaItem.toLocaleString('id-ID')}</b></span>
                </div>
              </td>
            </tr>
          )}
              ...
              <td>
                ...
                <div style={{ background: '#fefce8', border: '1px solid #fde68a', borderRadius: 8, padding: '8px 12px', marginTop: 6, fontSize: 13 }}>
                  <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', color: '#92400e' }}>
                    ...
                    <span>Total harga: <b style={{ color: '#059669' }}>Rp {totalHargaKotak.toLocaleString('id-ID')}</b></span>
                  </div>
                </div>
              </td>
            </tr>
          ...
          <tr>
            ...
            <td style={{ fontWeight: 700, color: '#059669', fontSize: 17 }}>Rp {Number(data.total).toLocaleString('id-ID')}</td>
          </tr>
          <tr>
      ...
      <button className="btn btn-primary" onClick={onKembali}>← Kembali</button>
    </div>
  );
}